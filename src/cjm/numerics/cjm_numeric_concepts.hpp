...
namespace cjm::numerics::concepts
{
	...
	/// </summary>
	template<typename T>
	concept can_find_most_significant_set_bitpos = builtin_unsigned_integer<T> 
		|| internal::has_static_fls_mem_func<T>;
	...
	concept builtin_128bit_integer = builtin_integer<Integer> &&
		std::numeric_limits<Integer>::digits == 128;

	template<typename Integer>
	concept builtin_128bit_unsigned_integer = unsigned_integer<Integer> &&
	...
	template<typename T>
	concept utf_character = character<T> &&
		(	std::is_same_v<T, char8_t> 	||
			std::is_same_v<T, char16_t>	||
			std::is_same_v<T, char32_t>	);
	...
	concept char_with_traits = character<Char> &&
		std::is_nothrow_convertible_v<CharTraits, std::char_traits<Char>>;

	template<typename Char, typename CharTraits>
	concept char_or_wchar_t_with_traits = char_or_wchar_t<Char> &&
		char_with_traits<Char, CharTraits>;

	template<typename Char, typename CharAllocator>
	concept char_with_allocator = character<Char> &&
		std::is_nothrow_convertible_v<std::allocator<Char>, CharAllocator>;

	template<typename Char, typename CharAllocator>
	concept char_or_wchar_t_with_allocator = char_or_wchar_t<Char> && 
		char_with_allocator<Char, CharAllocator>;

	template<typename Char, typename CharTraits, typename CharAllocator>
	concept char_with_traits_and_allocator = character<Char> && 
		char_with_traits<Char, CharTraits> && char_with_allocator<Char, CharAllocator>;

	template<typename Char, typename CharTraits, typename CharAllocator>
	concept char_or_wchar_t_with_traits_and_allocator = char_or_wchar_t_with_allocator<Char, CharAllocator> &&
...
#endif //INT128_CJM_NUMERIC_CONCEPTS_HPP_
